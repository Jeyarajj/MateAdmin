<template>
  <div>
    <v-toolbar flat color="white">
      ...
      <v-dialog v-model="dialog" persistent max-width="500px">
        <v-btn slot="activator" color="primary" dark class="mb-2">
          <v-icon left dark>add_circle</v-icon>Add New UserTypes</v-btn>
        <v-card>
          <v-card-title>
            <v-layout>
                <v-flex row xs6>
                  <span class="v-toolbar__title primary--text">{{ uformTitle }}</span>
                </v-flex>
    ...
    <v-data-table :headers="headers" :items="metaList" hide-actions class="elevation-1">
